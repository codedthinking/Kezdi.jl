...
@testset "Vectorize function calls" begin
    @test_expr vectorize_function_calls(:(log(x))) == :(Kezdi.BFA(log, x))
    @test_expr vectorize_function_calls(:(x + y)) == :(x .+ y) 
    @test_expr vectorize_function_calls(:(log(x) + log(z))) == :(Kezdi.BFA(log, x) .+ Kezdi.BFA(log, z))
    @test_expr vectorize_function_calls(:(div(x, y))) == :(Kezdi.BFA(div, x, y))
    @test_expr vectorize_function_calls(:(1 + div(x, y, z))) == :(1 .+ Kezdi.BFA(div, x, y, z))
    @testset "Do not vectorize" begin
        @test_expr vectorize_function_calls(:(mean(x))) == :(mean(skipmissing(x)))
        @test_expr vectorize_function_calls(:(mean(x) + log(y))) == :(mean(skipmissing(x)) .+ Kezdi.BFA(log, y))
        @test_expr vectorize_function_calls(:(log.(x))) == :(log.(x))
        @test_expr vectorize_function_calls(:(log(x) + sum(y))) == :(Kezdi.BFA(log, x) .+ sum(skipmissing(y)))
