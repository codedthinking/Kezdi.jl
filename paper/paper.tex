...
\texttt{Douglass.jl} is a related package that also aims to provide a Stata-like user experience in Julia \cite{Douglass.jl2023}. The two packages share similar goals, but \texttt{Kezdi.jl} has a more extensive feature set and is more actively developed. There are some differences between the two packages implementing Stata-like interface inside Julia. The main difference is between how Douglass and Kezdi process the commands. Douglass uses a string parsing approach where a string using Stata-like syntax, but using Julia conventions for referencing variables in a dataframe, is used to implement the actions on the dataframe whereas Kezdi uses macros and Julia's macro parsing and processing to implement all the actions. Other than this most of the differences are in the feature set provided by the packages. Including but not limited to: Kezdi's reference to dataframe variables is using the Stata convention rather than using Julia symbols, Kezdi has implemented commands for basic analysis of data matchin Stata commands and their syntax, on data manipulation commands both packages have some that the other does not implemented yet, and Douglass has implemented the \texttt{by:} and \texttt{bysort:} prefixes in general for commands.
